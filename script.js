...
function loadRandomImage() {
  return new Promise((resolve, reject) => {
    const imageUrls = [
      "https://vsuqgxrckqnxtdctjsgb.supabase.co/storage/v1/object/public/uploads/uploads/cpm35%202023-01-05%20100233.802%20pm.jpg",
      "https://vsuqgxrckqnxtdctjsgb.supabase.co/storage/v1/object/public/uploads/uploads/97a3a970-d7e6-11ef-b4b9-cb08e2f4af3d.jpg",
      "https://vsuqgxrckqnxtdctjsgb.supabase.co/storage/v1/object/public/uploads/uploads/image2.jpg",
    ];
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function setPosition(folder, topKey, leftKey) {
    if (localStorage.getItem(topKey) && localStorage.getItem(leftKey)) {
      folder.style.top = localStorage.getItem(topKey);
      folder.style.left = localStorage.getItem(leftKey);
    }
  }

  setPosition(folder1, 'folder1Top', 'folder1Left');
  setPosition(folder2, 'folder2Top', 'folder2Left');
  setPosition(folder3, 'folder3Top', 'folder3Left');
  setPosition(folder4, 'folder4Top', 'folder4Left');

  folder1.addEventListener('mousedown', (e) => {
    const rect = folder1.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;

    const moveFolder = (e) => {
      folder1.style.top = `${e.clientY - y}px`;
      folder1.style.left = `${e.clientX - x}px`;
      localStorage.setItem('folder1Top', folder1.style.top);
      localStorage.setItem('folder1Left', folder1.style.left);
    };

    document.addEventListener('mousemove', moveFolder);

    folder1.addEventListener('mouseup', () => {
      document.removeEventListener('mousemove', moveFolder);
    });
  });

  folder2.addEventListener('mousedown', (e) => {
    const rect = folder2.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;

    const moveFolder = (e) => {
      folder2.style.top = `${e.clientY - y}px`;
      folder2.style.left = `${e.clientX - x}px`;
      localStorage.setItem('folder2Top', folder2.style.top);
      localStorage.setItem('folder2Left', folder2.style.left);
    };

    document.addEventListener('mousemove', moveFolder);

    folder2.addEventListener('mouseup', () => {
      document.removeEventListener('mousemove', moveFolder);
    });
  });

  folder3.addEventListener('mousedown', (e) => {
    const rect = folder3.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;

    const moveFolder = (e) => {
      folder3.style.top = `${e.clientY - y}px`;
      folder3.style.left = `${e.clientX - x}px`;
      localStorage.setItem('folder3Top', folder3.style.top);
      localStorage.setItem('folder3Left', folder3.style.left);
    };

    document.addEventListener('mousemove', moveFolder);

    folder3.addEventListener('mouseup', () => {
      document.removeEventListener('mousemove', moveFolder);
    });
  });

  folder4.addEventListener('mousedown', (e) => {
    const rect = folder4.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;

    const moveFolder = (e) => {
      folder4.style.top = `${e.clientY - y}px`;
      folder4.style.left = `${e.clientX - x}px`;
      localStorage.setItem('folder4Top', folder4.style.top);
      localStorage.setItem('folder4Left', folder4.style.left);
    };

    document.addEventListener('mousemove', moveFolder);

    folder4.addEventListener('mouseup', () => {
      document.removeEventListener('mousemove', moveFolder);
    });
  });
